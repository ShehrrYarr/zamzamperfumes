...
private function internalRefundLineAmount(Sale $sale, float $lineSubtotal): float
    {
        ...
        if ($dtype === 'flat') {
            // prorate by discounted ratio
            if ($subtotal <= 0) return 0.0;
            $ratio = $grand / $subtotal; // e.g. (subtotal - flat)/subtotal
     ...
     private function postInternalReturnAccountingOrSkip(Sale $sale, SaleReturn $returnHeader, float $amount): void
    {
        ...
        // Idempotency: one accounting posting per return header
        ...
        // "First account per shop"
        $branchAccount = Account::query()
            ->where('shop_id', $branchShopId)
            ->orderBy('id')
            ->first();

        $mainAccount = Account::query()
            ->where('shop_id', $mainShopId)
            ->orderBy('id')
            ->first();

        if (!$branchAccount || !$mainAccount) {
            // Don't break return if accounts missing
            return;
        }

        $desc = "Internal Transfer Return | Sale #{$sale->id} | Return #{$returnHeader->id}";

        // Branch: CREDIT (reduces payable to main)
        AccountEntry::create([
        ...
        try {
            $return = DB::transaction(function () use ($shop, $user, $data) {
                ...
                foreach ($data['items'] as $row) {
                    ...
                    // STOCK
                    if ($isInternalTransfer) {
                        // decrease BRANCH stock (branch sending back)
                        $branchBatch = Batch::where('shop_id', $branchShopId)
                            ->where('barcode', $saleItem->barcode)
                            ->lockForUpdate()
                            ->first();
                    ...
                    $lineRefund = $isInternalTransfer
                        ? $this->internalRefundLineAmount($sale, $lineSubtotal) // ✅ includes 15% (or flat proration)
                ...
                // Recalc sale totals from remaining items
                $remainingItems = \App\Models\SaleItem::where('sale_id', $sale->id)->get();
                $newSubtotal = (float)$remainingItems->sum('line_total');
                ...
                // ✅ INTERNAL TRANSFER ACCOUNTS: reverse posting for the returned amount
